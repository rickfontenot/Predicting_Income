...
lognumeric <- adult %>% select(-categorical.explanatory,-income)
lognumeric[paste0('log_',names(lognumeric))] <- lapply(lognumeric, function(x) log(x+0.01))
lognumeric <- lognumeric %>% select(-age,-fnlwgt,-education_num,-capital_gain,-capital_loss,-hours_per_week)

hist.data.frame(lognumeric, nclass=20)

str(lognumeric)
library(rstatix)
shapiro.test(lognumeric$log_age)

qqnorm(adult$age)
qqnorm(lognumeric$log_age)

adult %>% ggplot(aes(sample = age)) + geom_qq() + geom_qq_line() + labs(y="Age")
lognumeric %>% ggplot(aes(sample = log_age)) + geom_qq() + geom_qq_line() + labs(y="Log(Age)") 

adult %>% ggplot(aes(sample = capital_gain)) + geom_qq() + geom_qq_line() + labs(y="capital_gain")
lognumeric %>% ggplot(aes(sample = log_capital_gain)) + geom_qq() + geom_qq_line() + labs(y="Log(capital_gain)") 
...
interaction_check_set$income <- interaction_check_set$income.binary
interaction_check_set <- interaction_check_set %>% select(-income.binary)

model <- glm(income ~.*.,family=binomial(link='logit'),data=interaction_check_set)
summary(model)

test_na_rm$IncomeProbability <- predict(model, newdata = test_na_rm, type = "response")

test_na_rm["Prediction"] = 0
test_na_rm$Prediction[test_na_rm$IncomeProbability>0.5] = 1
test_na_rm$Prediction=as.factor(test_na_rm$Prediction)
test_na_rm$income=as.factor(test_na_rm$income)

confusionMatrix(test_na_rm$Prediction, test_na_rm$income)
