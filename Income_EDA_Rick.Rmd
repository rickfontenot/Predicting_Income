...
logit.set <- adult %>% select(-fnlwgt,-work_sector,-marriage_status,-collar,-native_country,-birthplace)
logit.set <- na.omit(logit.set)
logit.set$income.binary<-0
logit.set$income.binary[logit.set$income==" >50K"] <- 1
logit.set$income.binary = as.factor(logit.set$income.binary)
...
```{r}
train_na_as_cat <- mutate_if(train_na_as_cat, is.character, ~ as.factor(as.factor(.x)))
train_na_as_cat <- mutate_if(train_na_as_cat, is.factor, ~ as.numeric(as.factor(.x)))

scale = preProcess(train_na_as_cat, method = "range")
scaled.train_na_as_cat = predict(scale, train_na_as_cat)
...
{
  fit <- class::knn(cl = scaled.train_na_as_cat$income, test = scaled.test_na_as_cat %>% select(knn.predictors), train = scaled.train_na_as_cat %>% select(knn.predictors), k=i, prob = TRUE)
  roc.perf <- roc(scaled.test_na_as_cat$income, attributes(fit)$prob)
...
optimalknn <- class::knn(cl = scaled.train_na_as_cat$income, test = scaled.test_na_as_cat %>% select(knn.predictors), train = scaled.train_na_as_cat %>% select(knn.predictors), k = 30, prob = TRUE)

plot(roc(scaled.test_na_as_cat$income, attributes(optimalknn)$prob), print.thres = T, print.auc=T, asp=1)

confusionMatrix(as.factor(optimalknn), as.factor(scaled.test_na_as_cat$income))
